...
class ClinicsHomeController extends Controller
{
    ...
    public function show(Consultation $clinics_home, Request $request)
    {
        ...
        if($datum->booking_type == 'Dialysis'){
            $prevBooking = Consultation::where('patient_id', $datum->patient_id)->where('booking_type', 'Dialysis')->whereNull('consultation_parent_id')->whereNot('id', $datum->id)->orderBy('bookingDate','desc')->first();
