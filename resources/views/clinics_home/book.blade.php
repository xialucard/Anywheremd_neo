...
<datalist id="doctorClinicNameList">
  ...
  @foreach($user->clinic->affiliated_doctors->sortBy('name') as $doc)
    ...
    @foreach($doc->doctor->affiliated_clinics->sortBy('name') as $clin)
      @foreach($doc->doctor->schedules()->whereBetween('dateSched', [$datum->bookingDate, date('Y-m-d', strtotime(' + 3 day'))])->orderBy('dateSched', 'asc')->distinct()->get('dateSched') as $sched)
